...
@Override
    ...
    public Element toElement(Document document) {
        ...
        libro.setAttribute("numero_de_paginas", String.valueOf(numero_de_paginas));

        // Elemento autores y sus hijos
        libro.appendChild(document.createElement("autores"));
        for (String autor : autores) {
            Element autorElement = document.createElement("autor");
            autorElement.setTextContent(autor);
            libro.getElementsByTagName("autores").item(0).appendChild(autorElement);
        }

        // Elemento fechaEdicion
        Element fechaEdicionElement = document.createElement("fechaEdicion");
        fechaEdicionElement.setTextContent(fechaEdicion.format(DateTimeFormatter.ofPattern("dd-MM-yyyy")));
        libro.appendChild(fechaEdicionElement);

        // Elemento editorial
        Element editorialElement = document.createElement("editorial");
        editorialElement.setTextContent(editorial);
        libro.appendChild(editorialElement);

        // Elemento precio
        Element precioElement = document.createElement("precio");
        precioElement.setTextContent(String.valueOf(precio));
        libro.appendChild(precioElement);

        // Elemento copias y sus hijos
        libro.appendChild(document.createElement("copias"));
        for (Copia copia : copias) {
            libro.getElementsByTagName("copias").item(0).appendChild(copia.toElement(document));
        }
